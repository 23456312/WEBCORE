...
<html>
  ...
  <body>
    ...
    <section class="section">
        <div class="container">
        <h1 class="title">
            Bienvenido a Ternu shop!
        </h1>`;

const html_content_form = `<div class="columns">
            <div class="column">
              <form action="/" method="POST">
                <label for="nombre" class="label">Nombre del ternurin</label>
                <input
                  class="input is-primary"
                  type="text"
                  placeholder="Ternurin ballena"
                  id="nombre"
                  name="nombre"
                />
                <br><br>
                <input type="submit" id="boton_personaje" class="button is-primary" value="Guardar ternurin">
              </form>
            </div>
        </div>
        `;
...
const server = http.createServer( (request, response) => {   
    if (request.url == "/" && request.method == "GET") {
      ...
      response.setHeader('Content-Type', 'text/html');
      response.write(html_header + "Este ternurin no existe" + html_footer);
      response.end();
    }
});
